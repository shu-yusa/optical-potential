...
!     Title  : Opt_pot.f90                                             !
!     Author : Yusa Shusaku                                            !
!     Date   : 2008-6-24-Tue ~ 2008-6-26-Thu                           !
!     Last modified : 2009-3-7-Sat                                     !
!                                                                      !
!     This program computes angular distribution of differential cross !
!     section for the elastic scattering. We use optical potential for !
!     the calculation and the shape of the potential is assumed to be  !
!     Woods-Saxon shape.                                               !
!     In this program, we compute for the scattering of ^12C by ^28Si  !
...
!     Elab  ... Bombering energy in laboratory.                        !
...
! *** Fitting parameters ***                                           !
!     Rc    ... Coulomb radius.                                        !
!     V0, a1, R1(rv) ... For real part of optical potential.           !
!     W0, a2, R2(rw) ... For imaginary part of optical potential.      !
!----------------------------------------------------------------------!
      implicit none
      ...
      integer, parameter :: Lc=k*rmax*(sqrt(1.0d0-2.0d0*eta/(k*rmax))) &
     &                         + 1.0d0
      real(8), parameter, private :: rv=1.323d0, rw=1.169d0
      real(8), parameter :: Rc=6.3912d0
      real(8), parameter :: V0=-10.0d0, W0=-29.1d0
      real(8), parameter :: a1=0.647d0, a2=0.614d0
      real(8), parameter :: R1=rv*(Apro**(1.0d0/3.0d0) + &
     &                            Atar**(1.0d0/3.0d0))
      real(8), parameter :: R2=rw*(Apro**(1.0d0/3.0d0) + &
     &                            Atar**(1.0d0/3.0d0))
      end module
!======================================================================!
      program main
...
!----------------------------------------------------------------------!
      ...
      call  Phase_Shift(Sn, sigma)
      call  Disp_diff_CS(sigma, Sn)
      call  Gnuplot(sigma, Sn, 7, 'gnuopt')

      stop
      end program
!======================================================================!
      subroutine  Phase_Shift(Sn, sigma)
!----------------------------------------------------------------------!
...
!     allowed to apply for complex values). Therefore, we compute the  !
...
!----------------------------------------------------------------------!
      ...
      complex(8), intent(out) :: Sn(0:Lc)
      complex(8), dimension(0:Lc) :: Hpa, Hma, Hpb, Hmb
      complex(8), parameter :: i=(0.0d0, 1.0d0)
      complex(8) :: s

      call Inn_Sol(0, a, b, s)
...
!----------------------------------------------------------------------!
      ...
      complex(8), external :: kk
      complex(8) :: y(3), ya, ynm1, ynm2
...
!----------------------------------------------------------------------!
      ...
      real(8), intent(in) :: x
      real(8), external :: Vc
      complex(8), external :: Vopt
      complex(8) :: k
      
      k = 2.0d0 * rmass * (E - Vopt(x) - Vc(x)) / (hbarc * hbarc)  &
     &        - dble((L * (L + 1))) / (x * x)
...
!----------------------------------------------------------------------!
      ...
      fn = (0.0d0, 0.0d0)
      do L=Lc, 0, -1
        Sc = exp(2.0d0*i*sigma(L))
        fn = fn + dble(2*L + 1) * (Sn(L) - 1.0d0) * PL(L,cos(t)) * Sc
      end do
      fn = - 0.5d0 * i / k * fn
...
!     distribution.                                                    !
!----------------------------------------------------------------------!
      use Com_var, only : Rc, Zpro, Ztar, hbarc
      implicit none
      real(8), intent(in) :: x
      real(8) :: V
...
!======================================================================!
      subroutine  Gnuplot(sigma, Sn, Fnum, Fname)
!----------------------------------------------------------------------!
...
!----------------------------------------------------------------------!
      ...
      write(Fnum,*) 'set output ''12C_on_28Si.eps'''
...
!              intent(out)                                             !
!     sigmad(1:L) --- Coulomb phase shift. Maybe... intent(out)        !
